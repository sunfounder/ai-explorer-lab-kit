...
- Connect ST_CP (Storage Register Clock) of 74HC595 to GPIO18 of the Raspberry Pi.
- Connect SH_CP (Shift Register Clock) to GPIO27.
- Connect DS (Data Input) to GPIO17.
...
.. image:: ../python/img/1.1.4_7_segment_schematic.png
...
.. image:: ../python/img/1.1.4_7-Segment_circuit.png
...
1. **Importing Libraries**
   ...
   The ``gpiozero`` library simplifies GPIO control, and the ``time`` library provides delays.
...
1. **7-Segment Display Not Working**  
   ...
   - **Cause**: Incorrect wiring between the Raspberry Pi, 74HC595, and 7-segment display.  
   - **Solution**: Verify that the GPIO pins (17, 18, 27) are correctly connected to ``SDI``, ``RCLK``, and ``SRCLK`` of the shift register, and that the output pins of the 74HC595 are connected to the display.
