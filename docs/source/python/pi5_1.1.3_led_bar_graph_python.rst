...
.. image:: ../python/img/1.1.3_LedBarGraph_schematic.png
...
.. image:: ../python/img/1.1.3_LedBarGraph_circuit.png
...
3. **GPIOZero Module Not Found**  

   - **Cause**: The ``gpiozero`` library is not installed.  
   - **Solution**: Install the library with ``pip install gpiozero``.
...
3. **PWM Control for Smooth Transitions**  

   Replace ``LED`` objects with ``PWMLED`` objects to control brightness for smoother lighting transitions.
